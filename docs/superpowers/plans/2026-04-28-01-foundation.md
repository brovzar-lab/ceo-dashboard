...
**Goal:** Bootstrap the CEO Dashboard monorepo — working `npm run dev` with Vite (:5173) + Express (:3001), all shared types, design tokens, and seed data rendering in the browser.
...
export function App() {
  return (
    <div className="min-h-screen bg-bg-base text-text-primary font-body">
      <h1 className="font-display text-2xl p-8">CEO Dashboard</h1>
    </div>
  )
}
```
...
Open browser at http://localhost:5173. Expected: Dark page with "CEO Dashboard" in Fraunces font.
